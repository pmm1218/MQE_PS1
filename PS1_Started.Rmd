...
```
### Group Member 1: Jacob Humble
### Group Member 2: Tricia McMahon
### Group Member 3: Max Thompson
...
**Answer:**
```{r sim3}
Reg1 <- lm(Y~ V1+V2, data=out)
summary (Reg1)
#These variable are not biased.
```
...
**Answer:**
```{r sim4}
Reg2 <- lm(Y~ V1, data=out)
summary (Reg2)
#The coefficient for the single regression is different than the coefficient from the multiple regression, thus it is biased.
```
...
**Answer:**
```{r sim5}
adjY <- out$Y-B2*out$V2
Reg3 <- lm(adjY ~ V1, data=out)
summary (Reg3)
#The estimate of beta_1 is not biased, as both are around 3.
...
plotted<-ggplot(out, aes(V1, y = value, color = variable)) + 
    geom_point(aes(y = Y, col = "Y"), color= 'lightblue3') + 
    geom_point(aes(y = adjY, col = "adjY"), color='gold3')+
    geom_smooth(method='lm', aes(y = Y, col = "Y"), color='gray67')+
    geom_smooth(method='lm', aes(y = adjY, col = "Y"), color='gray67') + 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
    panel.background = element_blank(), axis.line = element_line(colour = "black")) + theme_bare
